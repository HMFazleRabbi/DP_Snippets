...
int solve_recursive(const std::vector<char>& dims, int i, int j){
	...
	for (int k = i; k<j; ++k) {
        int left = m_array[i][k] != -1 ? m_array[i][k] : solve_recursive(dims, i, k); 
        int right = m_array[k+1][j] != -1 ? m_array[k+1][j] : solve_recursive(dims, k+1, j); 
...
int main()
{
	...
	std::cout <<"38 Palindrome Partitioning Memoized Optimisation:\t"<<solve_recursive(seq, 0, seq.size()-1)<<"\n";
    return 0;
}